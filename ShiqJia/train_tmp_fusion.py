...
def train(args):
    ...
    train_loader, val_loader = semantic_dataset_tf(args.version, args.dataroot, data_conf, args.bsz, args.nworkers, args.tf_size)
    ...
    for epoch in range(args.nepochs):
        for batchi, (imgs, trans, rots, intrins, post_trans, post_rots, car_trans, yaw_pitch_roll, semantic_gt, instance_gt, direction_gt, sample_token) in enumerate(train_loader): # lidar_data, lidar_mask,
            # print(enumerate(train_loader))
            t0 = time()
            opt.zero_grad()
            if torch.cuda.is_available():
                # points, points_mask, x, rots, trans, intrins, post_rots, post_trans, translation, yaw_pitch_roll
                # semantic, embedding, direction = model(lidar_data, lidar_mask, imgs.cuda(), rots.cuda(), trans.cuda(), intrins.cuda(),
                #                                     post_rots.cuda(), post_trans.cuda(), 
...
if __name__ == '__main__':
    ...
    parser.add_argument("--tf_size", type=int, default=3)
