...
class HDMapNetTemporalFusionDataset(HDMapNetSemanticDataset):
    ...
    def __getitem__(self, idx):
        ...
        """
        rec_tf = self.samples[idx]
        semantic_masks, instance_masks, _, _, direction_masks = self.get_semantic_map(rec_tf)        
        ...
        for i in range(0, self.tf_size):
            ...
            # _semantic_masks, _instance_masks, _, _, _direction_masks = self.get_semantic_map(rec)
            ...
            # semantic_masks.append(_semantic_masks)
            # instance_masks.append(_instance_masks)
            # direction_masks.append(_direction_masks)
        ...
        return imgs, trans, rots, intrins, post_trans, post_rots, car_trans, yaw_pitch_roll, semantic_masks, instance_masks, direction_masks, sample_token                     
        # Needed x, rots, trans, intrins, post_rots, post_trans, translation, yaw_pitch_roll,smp

def semantic_dataset_tf(version, dataroot, data_conf, bsz, nworkers, tf_size=1):
    ...
    train_dataset_tf = HDMapNetTemporalFusionDataset(version, dataroot, data_conf, tf_size, is_train=True)
    ...
    train_loader_tf = torch.utils.data.DataLoader(train_dataset_tf, batch_size=bsz, shuffle=True, num_workers=nworkers, drop_last=True)
    ...
    #tf
    return train_loader_tf, val_loader_tf
