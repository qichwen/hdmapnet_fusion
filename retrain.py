...
def train(args):
    ...
    # 加载预训练模型权重
    if args.resume and os.path.isfile(args.modelf):
        model.load_state_dict(torch.load(args.modelf), strict=False)
        logger.info(f"Loaded pretrained model from {args.modelf}")
...
if __name__ == '__main__':
    ...
    parser.add_argument('--resume', action='store_true', help='Resume training from a checkpoint if available')
