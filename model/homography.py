...
import cv2
from image_processer import visualizer_plt, init_tb_logger
...
def plane_grid(xbound, ybound, zs, yaws, rolls, pitchs, cuda=True):
    ...
    y = torch.linspace(xmin, xmax, abs(num_x))
    x = torch.linspace(ymin, ymax, abs(num_y))
    if cuda and torch.cuda.is_available():
        x = x.cuda()
        y = y.cuda()
...
def ipm_from_parameters(image, xyz, K, RT, samp_token, target_h, target_w, extrinsic, post_RT=None):
    """
    :param image: [B*N, H, W, C]
    ...
    if post_RT is not None:
        P = post_RT @ P
    #维度的大小是-1。在NumPy中，-1表示该维度的大小由其他维度的大小和整个数组的元素总数来自动计算
    P = P.reshape(-1, 4, 4)
    #image.shape[1:3] 表示取 image.shape 元组的第2个和第3个元素, output : H, W
    pixel_coords = perspective(xyz, P, target_h, target_w, extrinsic, image.shape[1:3])
    
    #(6, 100, 200, 2)
    #TODO: for visual
    # for view in range(1, pixel_coords.shape[0]+1):
    #     # pixel_coord = pixel_coords[view]
    #     # pixel_coord = torch.sum(pixel_coords.squeeze(0),0)
    #     #accept only B,C,H,W or C,H,W img!!!
    #     pixel_img = pixel_coords[(view-1)]
    #     # H 100, W 200, C 2
    #     pixel_img = pixel_img.detach().cpu().numpy().transpose(2,0,1)
    #     # visualizer_plt(x,counter, sample_token, sample_channel, ts, step)

    #     visualizer_plt(pixel_img, view , samp_token, const.CAMS[view-1],'', "IPM_pixel_coords")          

    # visualizer_plt(pixel_coords, sample_token='', sample_channel=str(view), ts="", counter=0)
...
class IPM(nn.Module):
    ...
    def forward(self, images, Ks, RTs,samp_token, translation, yaw_roll_pitch, post_RTs=None):
        ...
        warped_fv_images = ipm_from_parameters(images, planes, Ks, RTs,samp_token, self.h, self.w, self.extrinsic, post_RTs)
