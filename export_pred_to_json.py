...
def export_to_json(model, val_loader, angle_class, args):
    ...
    model.eval()
    with torch.no_grad():
        for batchi, (imgs, trans, rots, intrins, post_trans, post_rots, lidar_data, lidar_mask, car_trans, yaw_pitch_roll, segmentation_gt, instance_gt, direction_gt, sample_token) in enumerate(tqdm.tqdm(val_loader)):
