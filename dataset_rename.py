...
from .image import img_transform
...
class HDMapNetDataset(Dataset):
    ...
    def get_samples(self):
        ...
        # print(self.nusc.list_sample('be99ffc878b24aca8956bbb4e0f97d0c'))
    ...
    def get_imgs(self, rec):
        ...
        post_rots = []
        counter = 1
        #all data from each camera
        
        #get dict from json file, 
        if os.path.exists("cams_map.json"):
            with open("cams_map.json", "r") as file:  
                cam_map = json.load(file)
        else:
            print("Lack of config file for cams map")
            return
        ...
        for cam in CAMS:
            ...
            #resize eff and origin size, params? resize -> (), resize_dims ->const (1600,900)
            ...
            ts = samp['timestamp']
            # crop impled. from mbdataset already
            # img = img.crop((0, 70, 352, 128))
    ...
    def __getitem__(self, idx):
        ...
        return imgs, trans, rots, intrins, post_trans, post_rots
...
class HDMapNetSemanticDataset(HDMapNetDataset):
    ...
    def __getitem__(self, idx):
        ...
        #car_trans, yaw_pitch_roll = self.get_ego_pose(rec)
        #semantic_masks, instance_masks, _, _, direction_masks = self.get_semantic_map(rec)
        #return imgs, trans, rots, intrins, post_trans, post_rots, semantic_masks, instance_masks, direction_masks
        sample_token = rec.get('token')
        return imgs, trans, rots, intrins, post_trans, post_rots, sample_token
